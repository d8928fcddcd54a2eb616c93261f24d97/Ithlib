...
namespace ithkuil
{
	namespace v3
	{
		...
		inline auto serialize_vf(context c, format f) noexcept -> serialized_vf_t
		{
		...
		inline auto deserialize_vf(serialized_vf_t serialized) noexcept -> std::tuple<context, format>
		{
			context c;
			format f;

			f = static_cast<format>(serialized & 15);
			serialized >>= 4;
		...
		namespace newslot
		{
			...
			public:
				using packed_t = unsigned int;
				using value_t = std::tuple<context, format>;
				packed_t packed = 0;
