...
//#include "include/ithlib/v3/ir.h"
...
//				std::optional<std::tuple<context_t, format_t>> vf_;
...
//					} };
//
//					const v3::format_t f { state.vf_ ? std::get<1>(*state.vf_) : v3::format_t::none };
...
//					if (!state.inc_cases.empty() || f != v3::format_t::none)
//					{
//						return true;
//					}
//					return false;
...
//{
//	std::wstring form = fmt::format_t(L"{}", L"");
//	auto rfind = [](const auto& map, const auto& value)
//	{
//		return std::find_if(std::begin(map), std::end(map), [value](auto kv)
//			{
...
//					{
//						const auto [c, format_t] = xxx.vf_.value_or(std::make_tuple(v3::context_t::existential, v3::format_t::none));
//						f.inc_pc = xxx.inc_pc;
...
#include <fmt/format.h>
#include "include/ithlib/v3/abbreviations.h"
int main()
{
	fmt::print(NSTR("{}"), get<ithkuil::v3::configuration_t>(ithkuil::v3::abbreviation).right.at(ithkuil::v3::configuration_t::discrete));
	return 0;
}